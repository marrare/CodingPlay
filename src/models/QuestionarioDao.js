...
QuestionarioDao.prototype.buscarRespostaUsuario = function(valor,callback){
	this._connection.query("SELECT COUNT(resposta) AS qtd_resposta FROM respostas_questionario WHERE id_participa_sessao IN(SELECT id from participa_sessao WHERE id_participante = "+valor.id_participante+" AND id_sessao = "+valor.id_sessao+")", callback);
}
